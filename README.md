...
- Fetches directly from the browser, per location:
  - Weather + rainfall forecast: Open-Meteo forecast API, free, no API key. Uses Open-Meteo's automatic "best match" model selection per location — don't assume a specific named model (we previously and incorrectly labelled this "BOM ACCESS-G" in the footer for every city; that model is Australia-specific and doesn't even return data for Ho Chi Minh City when queried directly, so the label has been removed).
  - Sea level: Open-Meteo Marine API, free, no API key. Modelled, not an official tide table — coastlines on an enclosed bay/gulf (Melbourne/Port Phillip Bay, Brisbane/Moreton Bay, Adelaide/Gulf St Vincent) are a rougher estimate; open coastlines (Sydney, Perth, Ho Chi Minh City's Vũng Tàu/Cần Giờ) tend to be more reliable. The Marine API is an ocean grid model and returns `null` for points it has no coverage for — confirmed by testing Ho Chi Minh City's original Nhà Bè site (15km up the Saigon River): always null. Swapped it for Vũng Tàu, an open-coast point near the river mouth that does return real data. The app shows "—" rather than crashing on any future null reading.
